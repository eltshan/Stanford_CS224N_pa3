...
public class ClassifierBased implements CoreferenceSystem {
	...
	private static final Set<Object> ACTIVE_FEATURES = mkSet(
			new Object[] {
	...
	public FeatureExtractor<Pair<Mention, ClusteredMention>, Feature, Boolean> extractor = new FeatureExtractor<Pair<Mention, ClusteredMention>, Feature, Boolean>() {
		private <E> Feature feature(Class<E> clazz, Pair<Mention, ClusteredMention> input, Option<Double> count) {
			...
			// --Variables
			Mention onPrix = input.getFirst(); // the first mention (referred to
												// as m_i in the handout)
			Mention candidate = input.getSecond().mention; // the second mention
															// (referred to as
															// m_j in the
															// handout)
			Entity candidateCluster = input.getSecond().entity; // the cluster
																// containing
																// the second
																// mention
			RuleBased ruleFeature = new RuleBased();
			// --Features
			if (clazz.equals(Feature.ExactMatch.class)) {
				...
				// return new
				// Feature.HeadWordMatch(onPrix.headWord().equals(candidate.headWord()));
				return new Feature.HeadWordMatch(ruleFeature.sameMention(onPrix, candidate));
				/*
		...
		@Override
		protected void fillFeatures(Pair<Mention, ClusteredMention> input, Counter<Feature> inFeatures, Boolean output,
	...
	public void train(Collection<Pair<Document, List<Entity>>> trainingData) {
		...
		for (Pair<Document, List<Entity>> datum : trainingData) {
			...
			for (int i = 0; i < mentions.size(); i++) {
				...
				// logf("Mention %s (%d datums)", onPrix.toString(),
				// dataset.size() - oldSize);
			}
		...
		labels.add(true);
		// (print features)
		for (Triple<Feature, Boolean, Double> featureInfo : this.classifier.getTopFeatures(labels, 0.0, true, 100,
				true)) {
	...
	public List<ClusteredMention> runCoreference(Document doc) {
		...
		for (int i = 0; i < mentions.size(); i++) {
			...
			for (int j = i - 1; j >= 0; j--) {
				...
				boolean coreferent = classifier
						.classOf(new RVFDatum<Boolean, Feature>(extractor.extractFeatures(Pair.make(onPrix, cand))));
			...
			if (coreferentWith < 0) {
				...
				// log("Mention " + onPrix + " coreferent with " +
				// mentions.get(coreferentWith));
